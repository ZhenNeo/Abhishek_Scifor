...
class Calculator():
    ...
    def bind_keys(self):
        ...
        for key in self.operations:
            self.root.bind(str(key), lambda event,operator = key: self.append_operator(operator))
    ...
    def create_display_frame(self):
        frame = tk.Frame(self.root, height=225, bg=light_gray)
    ...
    def create_operator_buttons(self):
        ...
        for operators,symbol in self.operations.items():
            button = tk.Button(self.buttons_frame, text=symbol, bg=off_white, fg=label_color, font=default_font_style, 
                               borderwidth=0, command=lambda x=operators: self.append_operator(x))
